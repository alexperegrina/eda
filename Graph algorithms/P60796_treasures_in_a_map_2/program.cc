...
int distancia_mas_corta(vector<pair<int,int> > &distres) {
	int ret = distres[0].second;
	for(int i = 1; i < distres.size();++i) {
		if(ret > distres[i].second) ret = distres[i].second;	
	}
	return ret;
}
...
int main() {
	...
	if(distres.size() > 0) cout << "minimum distance: " << distancia_mas_corta(distres) << endl;
