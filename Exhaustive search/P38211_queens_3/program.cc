...
void reines1(v_int& reines, v_bool& mc, v_bool& md1, v_bool& md2, int n, int i,bool &trobat) {
    if (i == n) {escriu(reines,n);trobat = true;}
    else {
        for (int j = 0; j < n; ++j) {
            if (not mc[j] and not md1[diag1(n,i,j)] and not md2[diag2(n,i,j)] and not trobat) {
                reines[i] = j;
                mc[j] = true;
                md1[diag1(n,i,j)] = true;
                md2[diag2(n,i,j)] = true;
                reines1(reines,mc,md1,md2,n,i+1,trobat);
                mc[j] = false;
...
int main() {
    ...
    reines1(reines,mc,md1,md2,n,0,trobat);
